...
如上方示例所示，我们使用了 `args` 作为注入参数名，注入的内容为 `CommandArg()`，也就是**命令型消息命令后跟随的内容**。在这个示例中，我们获得的参数会被检查是否有效，对无效参数则会结束事件。
...
:::tip 提示
`:=` 是 Python 3.8 引入的新语法 [Assignment Expressions](https://docs.python.org/3/reference/expressions.html#assignment-expressions)，也称为海象表达式，可以在表达式中直接赋值。
:::
...
NoneBot 提供了多种依赖注入类型，可以获取不同的信息，具体内容可参考[内置依赖注入](#)。

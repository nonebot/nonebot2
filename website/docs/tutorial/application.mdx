...
在 [快速上手](./quick-start.md) 中，我们已经介绍了如何安装和使用 `nb-cli` 创建一个项目。在本节中，我们将简要介绍如何在不使用 nb-cli 的方式创建一个机器人项目的**最小实例**并启动。如果你想要了解 NoneBot2 的启动流程，也可以阅读本节。
...
配置文件用于存放 NoneBot2 运行所需要的配置项，使用 [`pydantic`](https://pydantic-docs.helpmanual.io/) 以及 [`python-dotenv`](https://saurabh-kumar.com/python-dotenv/) 来读取配置。配置项需符合 dotenv 格式，复杂类型数据需使用 JSON 格式填写。详情可见[配置项](../plugin-advance/config#dotenv)

在**项目文件夹**中创建一个 `.env` 文件，并写入以下内容（去除注释）:

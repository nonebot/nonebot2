...
顾名思义，事件响应器的 `事件处理函数装饰器` 是 [decorator](https://docs.python.org/3/glossary.html#term-decorator)，那么它的使用方法也同[函数定义](https://docs.python.org/3/reference/compound_stmts.html#function-definitions)中所展示的包装用法相同。例如：
...
事件响应器操作与事件处理函数装饰器类似，作为事件响应器 `Matcher` 的[类方法](https://docs.python.org/3/library/functions.html#classmethod)存在，因此事件响应器操作的调用方法也是 `Matcher.func()` 的形式。不过不同的是，事件响应器操作并不是装饰器，因此并不需要@进行标注。
...
如上方示例所示，我们使用 `weather` 响应器的 `finish` 操作方法向用户回复了 `天气是...` 并结束了事件处理流程。效果如下：
...
值得注意的是，在执行 `finish` 方法时，NoneBot 会在向用户发送消息内容后抛出 `FinishedException` 异常来结束事件响应流程。也就是说，在 `finish` 被执行后，后续的程序是不会被执行的（如果你需要回复用户消息但不想事件处理流程结束，可以使用注释的部分中展示的 `send` 方法）。
...
目前 NoneBot 提供了多种事件响应器操作，其中包括用于用户交互与流程控制两大类，具体内容可参考[事件处理进阶](#)。

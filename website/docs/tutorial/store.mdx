...
插件包名可以在商店插件卡片中找到，或者使用 `nb-cli` 搜索插件显示的详情中找到。安装完成后，需要自行加载插件，例如：在 `pyproject.toml` 中的 `[tool.nonebot]` 中的 `plugins` 添加 `<插件包名>`。
...
插件包名可以在商店插件卡片中找到，或者使用 `nb-cli` 搜索插件显示的详情中找到。卸载完成后，需要自行移除插件加载，例如：在 `pyproject.toml` 中的 `[tool.nonebot]` 中的 `plugins` 移除 `<插件包名>`。
...
请在你的**项目目录**下执行该命令。`nb-cli` 会自动安装适配器并将其添加到加载列表中。
...
适配器包名可以在商店适配器卡片中找到，或者使用 `nb-cli` 搜索适配器显示的详情中找到。安装完成后，需要自行加载适配器。
...
请注意 `nb-cli` 并不会在安装驱动器后修改项目所使用的驱动器，请自行参考[配置](#)章节修改驱动器。

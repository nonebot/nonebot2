...
- 对于**项目名**，最好统一以 `nonebot-plugin-` 开头，之后为您拟定的插件名字，词间用横杠 `-` 分隔）；
  - **项目名**应该用于代码仓库名称、PyPI 包的发布名称等；
  - 本文使用 `nonebot-plugin-{your-plugin-name}` 作为**项目名**；
- 对于**导入名**，最好与**项目名**一致，但词间用下划线 `_` 分隔，即统一以 `nonebot_plugin_` 开头，之后为您拟定的插件名字；
  - **导入名**用于程序导入使用，应为插件文件（夹）的名称；
  - 本文使用 `nonebot_plugin_{your_plugin_name}` 作为**导入名**；
...
[A-kirami/nonebot-plugin-template](https://github.com/A-kirami/nonebot-plugin-template) 就是一个可用的模板。
...
__plugin_meta__ = PluginMetadata(
    ...
    type="{插件分类}",
    # 发布必填，当前有效值可为 `application`（向聊天用户提供功能） 或 `library`（只向开发者提供功能）。
    ...
    # 支持的适配器集合，其中 `~` 在此处代表前缀 `nonebot.adapters.`，其余适配器亦按此格式表记。
    # 若插件可以保证支持当下及未来所有适配器（即仅使用基本适配器功能）可不填写，否则应该列出插件支持的适配器。
...
根据您选用的构建系统，在项目的 `pyproject.toml` 中填入必要信息后进行构建与发布。

:::tip 提示
不同构建工具的使用可能存在差别。本文仅以 [`pdm`](https://pdm.fming.dev/latest/usage/publish/),
[`poetry`](https://python-poetry.org/docs/libraries/#packaging), [`setuptools`](https://setuptools.pypa.io/en/latest/)
